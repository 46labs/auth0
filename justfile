...
_docker-go-tests:
	...
	set -euo pipefail
	echo "gofmt..."
	docker run --rm -v "$(pwd):/auth0" -w /auth0 golangci/golangci-lint:v2.7.2-alpine gofmt -l /auth0
	echo "go test..."
	docker run --rm -v "$(pwd):/auth0" -w /auth0 golangci/golangci-lint:v2.7.2-alpine go test -v /auth0/pkg/...
	echo "Linting Go..."
